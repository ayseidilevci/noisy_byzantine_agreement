...
class Sender(Program):
    ...
    def __init__(self, M: int = 5, p0: float = 0.05, p1: float = 0.005):
        self.M = M
        self.p0 = p0
        self.p1 = p1
    ...
    def run(self, context: ProgramContext):
        ...
        for alpha in range(self.M):
            ...
            # Leakage simulation: Flip 0 ↔ 1
            if m0 == 0 and random.random() < self.p0:
                m0 = 1 - m0
            elif m0 == 1 and random.random() < self.p1:
                m0 = 1 - m0

            if m1 == 0 and random.random() < self.p0:
                m1 = 1 - m1
            elif m1 == 1 and random.random() < self.p1:
                m1 = 1 - m1

            measurements.append((int(m0), int(m1)))
        ...
        return {"xS": xS, "m0": int(m0), "m1": int(m1), "checkSet": checkSet, "measurements": measurements}
...
class Receiver0(Program):
    ...
    def __init__(self, name: str, M: int = 5, mu: float = 0.272, p0: float = 0.05, p1: float = 0.005):
        self.name = name
        self.M = M
        self.mu = mu
        self.p0 = p0
    ...
    def run(self, context: ProgramContext):
        ...
        for _ in range(self.M):
            ...
            # Leakage simulation: Flip 0 ↔ 1
            if m_qubit == 0 and random.random() < self.p0:
                m_qubit = 1 - m_qubit
            elif m_qubit == 1 and random.random() < self.p1:
                m_qubit = 1 - m_qubit
        ...
        for alpha in range(self.M):
            if alpha not in checkSet and measurements[alpha] == 1:
                m_xx10.append(alpha)
            elif alpha in checkSet and measurements[alpha] == 1:
                m_0011.append(alpha)
            else:
                m_xx0x.append(alpha)
        ...
        if n_min <= l3:
            forged_check_set = m_xx10 + random.sample(m_xx0x, n_min)
            y0 = 1
            is_valid = True
        else:
            forged_check_set = m_xx10
            y0 = None
            is_valid = False

        # Cross-Calling Phase
        csocket_r1 = context.csockets[self.PEER_R1]
        csocket_r1.send((y0, forged_check_set))
        ...
        return {
            ...
            "xS_received": xS,
            "checkSet": checkSet,
            "forged_check_set": forged_check_set,
            "y": y0,
            "l_counts": (l1, l2, l3),
            "is_valid": is_valid
        }
...
class Receiver1(Program):
    ...
    def __init__(self, name: str, M: int = 5, mu: float = 0.272, lambda_: float = 0.94, p0: float = 0.05,
                 p1: float = 0.005):
        self.name = name
        self.M = M
        self.mu = mu
